...
MIN_RATIO      = float(os.environ.get("MIN_RATIO", "2.0"))
...
def find_htf_ob(opens, highs, lows, closes, direction):
    for start in range(0, 6):
        i1, i2, i3 = -(start+1), -(start+2), -(start+3)
        if direction == 1:
            if closes[i1] < opens[i1] and closes[i2] < opens[i2] and closes[i3] < opens[i3]:
                return max(highs[i1], highs[i2], highs[i3]), min(lows[i1], lows[i2], lows[i3])
        else:
            if closes[i1] > opens[i1] and closes[i2] > opens[i2] and closes[i3] > opens[i3]:
                return max(highs[i1], highs[i2], highs[i3]), min(lows[i1], lows[i2], lows[i3])
    return None, None

def calc_retry_risk(reentry_count, base_risk):
    if reentry_count == 1:
...
def process_htf(symbol, htf_interval="1h", swing_len=5):
    s      = get_state(symbol)
    df     = get_klines(symbol, htf_interval, limit=500)
    ...
    if s["htf_bos_valid"]:
        if s["htf_bos_dir"] == 1 and s["htf_ref_hl"] and current_close < s["htf_ref_hl"]:
            ...
            send_telegram(f"⚠️ <b>HTF Invalidado</b> — {symbol}\nBajo HL: {s['htf_ref_hl']:.4f}")
        elif s["htf_bos_dir"] == -1 and s["htf_ref_lh"] and current_close > s["htf_ref_lh"]:
            _reset_htf_state(s, "HTF Invalidated (Above LH)")
            send_telegram(f"⚠️ <b>HTF Invalidado</b> — {symbol}\nSobre LH: {s['htf_ref_lh']:.4f}")
    ...
    if bull_bos and not (s["htf_bos_dir"] == 1 and s["htf_bos_valid"]):
        s.update({
            ...
            "htf_ref_hl": last_hl, "htf_ref_lh": None,
            "waiting_hl": True, "waiting_lh": False,
            "waiting_hh_confirm": False, "waiting_ll_confirm": False,
            "confirmed_hh": last_hh, "confirmed_ll": None,
        ...
        _reset_ltf_state(s)
        print(f"  📈 BULLISH BOS | {symbol} | {last_hh:.4f}")
        send_telegram(f"📈 <b>BULLISH BOS</b> — {symbol}\nNivel: {last_hh:.4f}\nHTF OB: {ob_low:.4f} — {ob_high:.4f}")

    if bear_bos and not (s["htf_bos_dir"] == -1 and s["htf_bos_valid"]):
        s.update({
            "htf_bos_dir": -1, "htf_bos_level": last_ll, "htf_bos_valid": True,
            "htf_ref_lh": last_lh, "htf_ref_hl": None,
            "waiting_lh": True, "waiting_hl": False,
            "waiting_hh_confirm": False, "waiting_ll_confirm": False,
            "confirmed_ll": last_ll, "confirmed_hh": None,
        ...
        _reset_ltf_state(s)
        print(f"  📉 BEARISH BOS | {symbol} | {last_ll:.4f}")
        send_telegram(f"📉 <b>BEARISH BOS</b> — {symbol}\nNivel: {last_ll:.4f}\nHTF OB: {ob_low_b:.4f} — {ob_high_b:.4f}")
    ...
    if s["waiting_hh_confirm"] and s["htf_bos_dir"] == 1:
        if last_hh and s["confirmed_hh"] and last_hh > s["confirmed_hh"]:
            channel_bot = s["htf_ob_low"]
            channel_top = last_hh
            channel_mid = channel_bot + (channel_top - channel_bot) * 0.5
            s.update({
                "waiting_hh_confirm": False,
                "htf_channel_bot": channel_bot, "htf_channel_top": channel_top,
                "htf_channel_mid": channel_mid, "htf_channel_built": True,
                "htf_channel_active": False,
                "trade_state": "Channel Built — Waiting for 50% Retrace"
            })
            _reset_ltf_state(s)
            print(f"  🔲 Canal alcista | {symbol} | {channel_bot:.4f} — {channel_mid:.4f} — {channel_top:.4f}")
            send_telegram(f"🔲 <b>Canal alcista</b> — {symbol}\n▲ Top: {channel_top:.4f}\n◈ 50%: {channel_mid:.4f}\n▼ Bot: {channel_bot:.4f}")
    ...
    if s["waiting_ll_confirm"] and s["htf_bos_dir"] == -1:
        if last_ll and s["confirmed_ll"] and last_ll < s["confirmed_ll"]:
            channel_top = s["htf_ob_high"]
            channel_bot = last_ll
            channel_mid = channel_bot + (channel_top - channel_bot) * 0.5
            s.update({
                "waiting_ll_confirm": False,
                "htf_channel_top": channel_top, "htf_channel_bot": channel_bot,
                "htf_channel_mid": channel_mid, "htf_channel_built": True,
                "htf_channel_active": False,
                "trade_state": "Channel Built — Waiting for 50% Retrace"
            })
            _reset_ltf_state(s)
            print(f"  🔲 Canal bajista | {symbol} | {channel_bot:.4f} — {channel_mid:.4f} — {channel_top:.4f}")
            send_telegram(f"🔲 <b>Canal bajista</b> — {symbol}\n▲ Top: {channel_top:.4f}\n◈ 50%: {channel_mid:.4f}\n▼ Bot: {channel_bot:.4f}")

    # ── Activar canal cuando low/high toca el 50% (fiel al Pine) ─────────────
    if s["htf_channel_built"] and not s["htf_channel_active"] and s["htf_bos_valid"]:
        if s["htf_bos_dir"] == 1 and lows[-1] <= s["htf_channel_mid"]:
            s["htf_channel_active"] = True
            s["trade_state"]        = "Channel Active — 50% Validated"
            print(f"  ✅ Canal activo | {symbol} | low: {lows[-1]:.4f} <= mid: {s['htf_channel_mid']:.4f}")
            send_telegram(f"⬛ <b>Canal activo</b> — {symbol}\nLow tocó 50%: {lows[-1]:.4f}\nBuscando entrada LTF...")
        if s["htf_bos_dir"] == -1 and highs[-1] >= s["htf_channel_mid"]:
            s["htf_channel_active"] = True
            s["trade_state"]        = "Channel Active — 50% Validated"
            print(f"  ✅ Canal activo | {symbol} | high: {highs[-1]:.4f} >= mid: {s['htf_channel_mid']:.4f}")
            send_telegram(f"⬛ <b>Canal activo</b> — {symbol}\nHigh tocó 50%: {highs[-1]:.4f}\nBuscando entrada LTF...")
...
def process_ltf(symbol, ltf_interval="5m", swing_len=3, fifty_tolerance=5.0):
    ...
    if s["ltf_state"] == 0:
        if in_fifty:
            ...
            print(f"    🎯 LTF 50% | {symbol} ({current_pos:.1f}%)")
        return None
    ...
    if s["ltf_state"] == 1:
        ...
        if bos_dir == 1 and s["ltf_internal_high"]:
            ...
            if c1 and c2 and c3 and prog and closes[-1] > s["ltf_internal_high"]:
                ...
                s.update({
                    "ltf_state": 2, "ltf_mss_level": s["ltf_internal_high"],
                    "ltf_mss_high": mss_high, "ltf_mss_low": mss_low,
                    "ltf_fifty_level": fifty_level, "ltf_fifty_hit": False,
                    "trade_state": "LTF MSS Bullish — Waiting for 50%"
                })
                print(f"    📈 LTF MSS Bull | {symbol} | 50%: {fifty_level:.4f}")
        ...
        if bos_dir == -1 and s["ltf_internal_low"]:
            ...
            if c1 and c2 and c3 and prog and closes[-1] < s["ltf_internal_low"]:
                ...
                fifty_level = mss_low + (mss_high - mss_low) * 0.5
                s.update({
                    "ltf_state": 2, "ltf_mss_level": s["ltf_internal_low"],
                    "ltf_mss_low": mss_low, "ltf_mss_high": mss_high,
                    "ltf_fifty_level": fifty_level, "ltf_fifty_hit": False,
                    "trade_state": "LTF MSS Bearish — Waiting for 50%"
                })
                print(f"    📉 LTF MSS Bear | {symbol} | 50%: {fifty_level:.4f}")
    ...
    if s["ltf_state"] == 2 and not s["ltf_fifty_hit"] and s["ltf_fifty_level"]:
        if bos_dir == 1 and lows[-1] <= s["ltf_fifty_level"]:
            ...
            print(f"    ✅ LTF 50% alcanzado | {symbol}")
    ...
    if s["ltf_state"] == 3 and s["ltf_fifty_hit"]:
        if bos_dir == 1:
            ...
            if closes[-1] > opens[-1] and closes[-2] < opens[-2] and s["ltf_ob_high"]:
                ...
                print(f"    📦 LTF OB alcista | {symbol} | {s['ltf_ob_low']:.4f} — {s['ltf_ob_high']:.4f}")
                send_telegram(f"📦 <b>OB LTF</b> — {symbol}\n{s['ltf_ob_low']:.4f} — {s['ltf_ob_high']:.4f}\n⏳ Esperando entrada...")
        ...
        if bos_dir == -1:
            ...
            if closes[-1] < opens[-1] and closes[-2] > opens[-2] and s["ltf_ob_low"]:
                ...
                s["ltf_state"]        = 4
                s["trade_state"]      = "LTF OB Identified — Waiting for Entry"
                print(f"    📦 LTF OB bajista | {symbol} | {s['ltf_ob_low']:.4f} — {s['ltf_ob_high']:.4f}")
                send_telegram(f"📦 <b>OB LTF</b> — {symbol}\n{s['ltf_ob_low']:.4f} — {s['ltf_ob_high']:.4f}\n⏳ Esperando entrada...")
    ...
    if s["ltf_state"] == 4 and not s["ltf_trade_active"]:
        ...
        if bos_dir == 1 and lows[-1] <= s["ltf_ob_high"] and highs[-1] >= s["ltf_ob_low"]:
            ...
            if ratio < MIN_RATIO:
                print(f"    ⚠️ Rechazado | {symbol} | Ratio {ratio:.2f}:1 < {MIN_RATIO}:1")
                send_telegram(f"⚠️ <b>Trade rechazado</b> — {symbol}\nRatio: {ratio:.2f}:1 (mínimo {MIN_RATIO}:1)")
                return None
            ...
            print(f"    🟢 LONG R{s['ltf_reentry_count']} | {symbol} | Entry: {entry:.4f} | SL: {sl:.4f} | TP: {tp:.4f} | Ratio: {ratio:.2f}:1")
            return {"side": "BUY", "entry": entry, "sl": round(sl, 6), "tp": round(tp, 6), "ratio": ratio}
        ...
        if bos_dir == -1 and highs[-1] >= s["ltf_ob_low"] and lows[-1] <= s["ltf_ob_high"]:
            ...
            print(f"    🔴 SHORT R{s['ltf_reentry_count']} | {symbol} | Entry: {entry:.4f} | SL: {sl:.4f} | TP: {tp:.4f} | Ratio: {ratio:.2f}:1")
            return {"side": "SELL", "entry": entry, "sl": round(sl, 6), "tp": round(tp, 6), "ratio": ratio}
    ...
    if s["ltf_state"] == 5 and s["ltf_trade_active"]:
        ...
        if not s["htf_bos_valid"]:
            print(f"    🚨 Emergency exit | {symbol}")
            log_trade_exit(symbol, side, s["ltf_entry_price"], closes[-1],
                           s["ltf_quantity"] or 0, s["ltf_balance"] or 0, "EMERGENCY EXIT")
            send_telegram(f"🚨 <b>EMERGENCY EXIT</b> — {symbol}\nHTF invalidado\nExit: {closes[-1]:.4f}")
            _reset_ltf_state(s)
            s["ltf_state"] = 0
            return None
        ...
        if bos_dir_current == 1:
            if lows[-1] <= s["ltf_stop_loss"]:
                ...
                log_trade_exit(symbol, side, s["ltf_entry_price"], lows[-1],
                               s["ltf_quantity"] or 0, s["ltf_balance"] or 0, f"STOP LOSS R{count}")
                send_telegram(f"❌ <b>STOP LOSS R{count}</b> — {symbol}\n{s['ltf_entry_price']:.4f} → {lows[-1]:.4f}\nRe-entry disponible")
                _reset_ltf_state(s)
                ...
                log_trade_exit(symbol, side, s["ltf_entry_price"], highs[-1],
                               s["ltf_quantity"] or 0, s["ltf_balance"] or 0, f"TAKE PROFIT R{count}")
                send_telegram(f"✅ <b>TAKE PROFIT R{count}</b> — {symbol}\n{s['ltf_entry_price']:.4f} → {highs[-1]:.4f} 🎯")
                _reset_ltf_state(s)
                s["ltf_state"] = 1
...
def set_leverage(symbol, leverage):
    try:
        ...
        print(f"  ⚙️ Leverage {leverage}x | {symbol}")
    except Exception as e:
        print(f"  ⚠️ Error leverage {symbol}: {e}")
...
def execute_trade(symbol, side, sl_price, tp_price, reentry_count=1):
    ...
    if ratio < MIN_RATIO:
        print(f"  ⚠️ Rechazado en ejecución | {symbol} | Ratio {ratio:.2f}:1")
        send_telegram(f"⚠️ <b>Trade rechazado</b> — {symbol}\nRatio: {ratio:.2f}:1")
        return {"status": "rechazado", "reason": "ratio insuficiente", "ratio": ratio}

    effective_risk = calc_retry_risk(reentry_count, RISK_PERCENT)
    print(f"  💰 R{reentry_count} | Riesgo: {effective_risk:.1f}% | Leverage: {LEVERAGE}x")

    sl_pct      = abs(entry_price - sl_price) / entry_price * 100
    risk_amount = balance * (effective_risk / 100)
    quantity    = (risk_amount * LEVERAGE) / (entry_price * sl_pct / 100)
    ...
    print(f"✅ {side} {quantity} {symbol} | Balance: {balance:.2f} | SL: {sl_price} | TP: {tp_price} | Ratio: {ratio:.2f}:1")
    ...
    send_telegram(
        ...
        f"R{reentry_count} | Riesgo: {effective_risk:.1f}% | Leverage: {LEVERAGE}x\n"
...
def index():
    return jsonify({"status": "bot activo", "leverage": LEVERAGE, "min_ratio": MIN_RATIO}), 200
...
def send_signal(symbol, signal):
    ...
    try:
        ...
        print(f"📤 Señal enviada | {symbol}: {r.json()}")
    except Exception as e:
...
def run_engine(
    ...
    print(f"🚀 Motor iniciado | {len(symbols)} pares | Leverage: {LEVERAGE}x | Min Ratio: {MIN_RATIO}:1")
    ...
    while True:
        ...
        print(f"\n⏳ Próxima vela en {wait_time:.0f}s ({next_candle})")
        time.sleep(wait_time)
        time.sleep(2)

        for symbol in symbols:
            try:
                print(f"\n📊 [{symbol}] {get_state(symbol)['trade_state']}")
...
if __name__ == "__main__":
    ...
    telegram_bot.init(state, client)

    engine_thread = threading.Thread(target=run_engine, daemon=True)
    engine_thread.start()

    tg_thread = threading.Thread(target=telegram_bot.start_polling, daemon=True)
    tg_thread.start()
